...
def get_max_metrics_hourly():
    metrics = [
        ...
        ('CmdGets', 'Maximum', SECONDS_IN_HOUR),
        ...
        ('CmdSet', 'Maximum', SECONDS_IN_HOUR),
...
def get_max_metrics_weekly():
    metrics = [
        ...
        ('BytesUsedForCacheItems', 'Maximum', SECONDS_IN_DAY * METRIC_COLLECTION_PERIOD_DAYS),
...
def get_clusters_info(session):
    ...
    results = {
        'mcd_running_instances': {},
        'mcd_reserved_instances': {},
    }
    ...
    for page in page_iterator:
        for instance in page['CacheClusters']:
            if (instance['CacheClusterStatus'] == 'available' and instance['Engine'] == 'memcached'):
                cluster_id = instance['CacheClusterId']
                results['mcd_running_instances'][cluster_id] = instance
    ...
    for page in page_iterator:
        for reserved_instance in page['ReservedCacheNodes']:
            if (reserved_instance['State'] == 'active' and reserved_instance['ProductDescription'] == 'memcached'):
                ...
                results['mcd_reserved_instances'][instance_type] = {
...
def create_workbook(outDir, section, region_name):
    ...
    df_columns = ["Source", "ClusterId", "NodeId", "CacheNodes", "NodeType", "Region"]
...
def get_running_instances_metrics(wb, clusters_info, session):
    ...
    running_instances = clusters_info['mcd_running_instances']
    ...
    for instanceId, instanceDetails in running_instances.items():
        for node in instanceDetails.get('CacheNodes'):
            ...
            row.append("%s-%s" % (clusterId, node.get('CacheNodeId')))
            row.append("%s" % instanceDetails.get('NumCacheNodes'))
...
def get_reserved_instances_info(wb, clusters_info):
    reserved_instances = clusters_info['mcd_reserved_instances']        
