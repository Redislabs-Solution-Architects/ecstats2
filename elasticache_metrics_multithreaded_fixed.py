...
# Thread configuration - will be updated by command line args
DEFAULT_MAX_ACCOUNT_THREADS = int(os.environ.get("MAX_ACCOUNT_THREADS", 1))
DEFAULT_MAX_CLUSTER_THREADS = int(os.environ.get("MAX_CLUSTER_THREADS", 5))
...
def process_single_node(args):
    ...
    try:
        thread_safe_print(f"Processing node {instance_id}:{node.get('CacheNodeId')}")
        ...
        thread_safe_print(f"Completed processing node {instance_id}:{node.get('CacheNodeId')}")
        return row
        
    except Exception as e:
        thread_safe_print(f"Error processing node {instance_id}:{node.get('CacheNodeId')}: {str(e)}")
        return None


def get_running_instances_metrics(wb, clusters_info, session, max_cluster_threads):
    """
    Get all the metrics for the clusters in the given set of clusters using multithreading
    Args:
        The cluster information dictionary, session, and max_cluster_threads
    Returns:
    """
    ...
    thread_safe_print(f"Processing {len(tasks)} nodes using {max_cluster_threads} threads")
    ...
    with ThreadPoolExecutor(max_workers=max_cluster_threads) as executor:
        ...
        # Collect results and write to worksheet
            ...
            try:
                ...
                if result is not None:
                    ...
                    with excel_lock:
                        ws.append(result)
                completed_count += 1
                thread_safe_print(f"Completed {completed_count}/{len(tasks)} nodes")
                
            except Exception as e:
                thread_safe_print(f"Task failed: {str(e)}")

    return wb


def get_reserved_instances_info(wb, clusters_info):
    """Get reserved instances info (no threading needed here as it's typically fast)"""
    reserved_instances = clusters_info["elc_reserved_instances"]
    ws = wb[RESERVED_INSTANCES_WORKSHEET_NAME]
    for instance_id, instance_details in reserved_instances.items():
        with excel_lock:
            ws.append(
                [
                    ("%s" % instance_id),
                    ("%s" % instance_details["count"]),
                    ("%s," % instance_details["expiry_time"]),
                ]
            )
    return wb


def process_aws_account(args):
    """Process a single AWS account - modified to work with thread pool"""
    config, section, out_dir, max_cluster_threads = args
    
    try:
        thread_safe_print(f"Starting processing for account: {section}")
        ...
        thread_safe_print(f"Requesting cluster information for {section}")
        clusters_info = get_clusters_info(session)

        wb = create_workbook(out_dir, section, region_name)
        wb = get_running_instances_metrics(wb, clusters_info, session, max_cluster_threads)
        wb = get_reserved_instances_info(wb, clusters_info)

        output_file_path = "%s/%s-%s.xlsx" % (out_dir, section, region_name)
        thread_safe_print(f"Writing output file {output_file_path}")
        
        # Thread-safe file write
        with excel_lock:
            wb.save(output_file_path)
            
        thread_safe_print(f"Completed processing for account: {section}")
...
def main():
    ...
    parser.add_option(
        ...
        help=f"Maximum number of threads for processing AWS accounts in parallel. Default: {DEFAULT_MAX_ACCOUNT_THREADS}",
    )
    parser.add_option(
        "--max-cluster-threads",
        ...
        help=f"Maximum number of threads for processing clusters/nodes in parallel. Default: {DEFAULT_MAX_CLUSTER_THREADS}",
    )

    (options, _) = parser.parse_args()

    # Use the options directly without global declaration
    max_account_threads = options.maxAccountThreads
    max_cluster_threads = options.maxClusterThreads

    print(f"Thread configuration:")
    print(f"  - Max account threads: {max_account_threads}")
    ...
    # Prepare tasks for parallel processing of AWS accounts
    account_tasks = []
    for section in config.sections():
        account_tasks.append((config, section, options.outDir, max_cluster_threads))

    print(f"Processing {len(account_tasks)} AWS accounts using {max_account_threads} threads")
